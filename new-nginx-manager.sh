...
# --- ЦВЕТА (через tput для надёжности) ---
...
# Функция управления UFW (портами)
manage_ufw() {
    echo "======================================"
    ...
    if ! command -v ufw &> /dev/null; then
        ...
        if [[ "$install_ufw" == "y" || "$install_ufw" == "Y" ]]; then
            ...
            # Открываем SSH порт 22, чтобы не потерять доступ
            ufw allow 22/tcp
...
# Настройка Certbot hooks (ufw 80)
...
menu() {
    ...
    echo "7. Настроить Certbot hooks (ufw: открыть/закрыть порт 80)"
